...
static void jobmetrics_list_register (const char *name, const char *jobId, const char *regexp)
{
	procstat_t *new;
	procstat_t *ptr;
	int status;
	...
	if (regexp != NULL)
	{
		ERROR ("jobmetrics plugin: jobmetrics_list_register: "
	...
	for (ptr = list_head_g; ptr != NULL; ptr = ptr->next)
	{
		if (strcmp (ptr->jobId, jobId) == 0)
		{
			WARNING ("jobmetrics plugin: You have configured more "
					"than one `Process' or "
					"`ProcessMatch' with the same jobId. "
...
static int jobmetrics_list_match (const char *jobId, const char *cmdline, procstat_t *ps)
{
#if HAVE_REGEX_H
	if (ps->re != NULL)
	{
...
static void jobmetrics_list_add (const char *jobId, const char *cmdline, procstat_entry_t *entry)
{
	procstat_t *ps;
	...
	{
		if ((jobmetrics_list_match (jobId, cmdline, ps)) == 0)
			continue;
...
int jobmetrics_read_process (int pid, procstat_t *ps, char *state)
{
	...
	if (fields_len < 22)
	{
		DEBUG ("jobmetrics plugin: ps_read_process (pid = %i):"
				" `%s' has only %i fields..",
...
#if KERNEL_SOLARIS
static const char *jobmetrics_get_cmdline (long pid, /* {{{ */
		char *buffer, size_t buffer_size)
{
	...
	{
		ERROR ("jobmetrics plugin: Unexpected return value "
				"while reading \"%s\": "
 ...
 */
static int jobmetrics_read_process(long pid, procstat_t *ps, char *state)
{
	char filename[64];
...
static int read_fork_rate()
{
	...
	jobmetrics_submit_fork_rate (result);
	return (0);
}
#endif /* KERNEL_SOLARIS */
...
static int jobmetrics_read (void)
{
		...
		for (task = 0; task < task_list_len; task++)
		{
						...
						task_name, PROCSTAT_NAME_LEN) == 0)
			{
				...
				for (ps = list_head_g; ps != NULL; ps = ps->next)
					...
					if (jobmetrics_list_match (task_name, NULL, ps) == 1)
						break;
			}
			...
			if (ps != NULL)
				/* FIXME: cmdline should be here instead of NULL */
				jobmetrics_list_add (task_name, NULL, &pse);
		} /* for (task_list) */
	...
	for (ps = list_head_g; ps != NULL; ps = ps->next)
		jobmetrics_submit_proc_list (ps);
/* #endif HAVE_THREAD_INFO */
...
/* #endif KERNEL_LINUX */
	...
	for (i = 0; i < count; i++)
	{
		...
		if ((proc_ptr == NULL) || (proc_ptr->ki_pid != procs[i].ki_pid))
		{
			...
			jobmetrics_list_add (procs[i].ki_comm, have_cmdline ? cmdline : NULL, &pse);
		} /* if ((proc_ptr == NULL) || (proc_ptr->ki_pid != procs[i].ki_pid)) */
	...
	jobmetrics_submit_state ("stopped",  stopped);
	jobmetrics_submit_state ("blocked",  blocked);
	jobmetrics_submit_state ("idle",     idle);
	jobmetrics_submit_state ("wait",     wait);
...
/* #endif HAVE_LIBKVM_GETPROCS && HAVE_STRUCT_KINFO_PROC_FREEBSD */
	...
	if (kd == NULL)
	{
		ERROR ("jobmetrics plugin: Cannot open kvm interface: %s",
				errbuf);
		return (0);
	}
	...
	if (procs == NULL)
	{
		ERROR ("jobmetrics plugin: Cannot get kvm processes list: %s",
				kvm_geterr(kd));
		kvm_close (kd);
		return (0);
	}
	...
	for (i = 0; i < count; i++)
	{
		...
		if ((proc_ptr == NULL) || (proc_ptr->p_pid != procs[i].p_pid))
		{
			...
			jobmetrics_list_add (procs[i].p_comm, have_cmdline ? cmdline : NULL, &pse);
		} /* if ((proc_ptr == NULL) || (proc_ptr->p_pid != procs[i].p_pid)) */
	...
	jobmetrics_submit_state ("zombies",  zombies);
	jobmetrics_submit_state ("stopped",  stopped);
	jobmetrics_submit_state ("onproc",   onproc);
	jobmetrics_submit_state ("idle",     idle);
	jobmetrics_submit_state ("dead",     dead);
...
/* #endif HAVE_LIBKVM_GETPROCS && HAVE_STRUCT_KINFO_PROC_OPENBSD */
					...
					&pindex, MAXPROCENTRY)) > 0)
	{
		...
		for (i = 0; i < nprocs; i++)
		{
			...
			jobmetrics_list_add (cmdline, cargs, &pse);
		} /* for (i = 0 .. nprocs) */
...
/* #endif HAVE_PROCINFO_H */
	...
	jobmetrics_submit_state ("zombies",  zombies);
	jobmetrics_submit_state ("stopped",  stopped);
	jobmetrics_submit_state ("detached", detached);
	jobmetrics_submit_state ("daemon",   daemon);
	jobmetrics_submit_state ("system",   system);
	jobmetrics_submit_state ("orphan",   orphan);
...
{
	plugin_register_init ("processes", jobmetrics_init);
	plugin_register_read ("processes", jobmetrics_read);
